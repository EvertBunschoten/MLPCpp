...
def Translate_Tensorflow_MLP(
    file_out: str,
    input_names: list[str],
    output_names: list[str],
    model: tf.keras.models.Sequential,
    scaler_input: str = "minmax",
    input_norm_1: list[float] = [],
    input_norm_2: list[float] = [],
    scaler_output: str = "minmax",
    output_norm_1: list[float] = [],
    output_norm_2: list[float] = [],
):
    ...
    if not n_inputs == len(input_names):
        raise Exception(
            "Number of provided input names unequal to the number of inputs in the model"
        )
    if not n_outputs == len(output_names):
        raise Exception(
            "Number of provided output names unequal to the number of outputs in the model"
        )

    if len(input_norm_2) != len(input_norm_1):
        raise Exception(
            "Upper and lower input normalizations should have the same length"
        )
    if len(output_norm_2) != len(output_norm_1):
        raise Exception(
            "Upper and lower output normalizations should have the same length"
        )
    ...
    for iLayer in range(n_layers - 1):
        ...
        if layer_class == "InputLayer":
            ...
            activation_functions.append(
                model_config["layers"][iLayer]["config"]["activation"]
            )
...
for NN in hidden_layers:
    model.add(
        tf.keras.layers.Dense(NN, activation="sigmoid", kernel_initializer="he_uniform")
    )
model.add(tf.keras.layers.Dense(1, activation="linear"))

# Set up training method
lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
    10**-2, decay_steps=1000, decay_rate=0.99, staircase=False
)
opt = tf.keras.optimizers.Adam(
    learning_rate=lr_schedule, beta_1=0.9, beta_2=0.999, epsilon=1e-08, amsgrad=False
)
...
Translate_Tensorflow_MLP(
    file_out="MLP_test",
    input_names=["u", "v"],
    output_names=["y"],
    model=model,
    scaler_input=scaler_function_x,
    input_norm_1=input_norm_1,
    input_norm_2=input_norm_2,
    scaler_output=scaler_function_y,
    output_norm_1=output_norm_1,
    output_norm_2=output_norm_2,
)
...
ax.plot3D(
    X_dim[:, 0], X_dim[:, 1], Y_dim_pred_MLPCpp, "g.", label="Model prediction (MLPCpp)"
)
ax.set_xlabel("u")
ax.set_ylabel("v")
ax.set_zlabel("y")
