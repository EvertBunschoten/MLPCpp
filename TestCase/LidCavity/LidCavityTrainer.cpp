...
bool ReadSU2CSV_NonDim(const std::string &csv_filename,
                       ...
                       std::vector<std::vector<mlpdouble>> &Y, double U_ref,
                       double p_ref) {
  ...
  while (std::getline(header_ss, col_name, ',')) {
    if (!col_name.empty() && col_name.front() == '"')
      col_name.erase(0, 1);
    if (!col_name.empty() && col_name.back() == '"')
      col_name.pop_back();
    headers.push_back(col_name);
  }

  int idx_x = -1, idx_y = -1, idx_rho = -1, idx_rhou = -1, idx_rhov = -1,
      idx_E = -1;
  for (size_t i = 0; i < headers.size(); ++i) {
    ...
    if (headers[i] == "Density")
      idx_rho = i;
    if (headers[i] == "Momentum_x")
      idx_rhou = i;
    if (headers[i] == "Momentum_y")
      idx_rhov = i;
    if (headers[i] == "Energy")
      idx_E = i;
  }

  if (idx_x == -1 || idx_y == -1 || idx_rho == -1 || idx_rhou == -1 ||
      idx_rhov == -1 || idx_E == -1) {
  ...
  while (std::getline(file, line)) {
    ...
    if (row_data.size() >
        std::max({idx_x, idx_y, idx_rho, idx_rhou, idx_rhov, idx_E})) {
      ...
      double p_dim =
          (gamma - 1.0) *
          (row_data[idx_E] - 0.5 * rho * (u_dim * u_dim + v_dim * v_dim));

      // Non-dimensionalize targets for the neural network
      Y.push_back({mlpdouble(u_dim / U_ref), mlpdouble(v_dim / U_ref),
...
int main(int argc, char **argv) {
  ...
  net.SetInputName(1, "y");
  net.SetOutputName(0, "u");
  ...
  eq_cont.residual = [sdf_weight](const PhysicsState &s,
                                  const PhysicsData &) -> mlpdouble {
    mlpdouble res = s.Jac(0, 0) + s.Jac(1, 1);
    return res * mlpdouble(sdf_weight(s));
  };
  auto loss_cont = std::make_shared<CPhysicsLoss>(
      "loss_continuity", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq_cont});

  // Eq 2: X-Momentum: u*du/dx + v*du/dy + C_p*dp/dx - (1/Re)*(d2u/dx2 +
  // d2u/dy2) = 0
  ...
  eq_xmom.residual = [Re, C_p, sdf_weight](const PhysicsState &s,
                                           const PhysicsData &) -> mlpdouble {
    mlpdouble u = s.Out(0);
    mlpdouble v = s.Out(1);
  ...
  auto loss_xmom = std::make_shared<CPhysicsLoss>(
      "loss_x_momentum", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq_xmom});

  // Eq 3: Y-Momentum: u*dv/dx + v*dv/dy + C_p*dp/dy - (1/Re)*(d2v/dx2 +
  // d2v/dy2) = 0
  ...
  eq_ymom.residual = [Re, C_p, sdf_weight](const PhysicsState &s,
                                           const PhysicsData &) -> mlpdouble {
    mlpdouble u = s.Out(0);
    mlpdouble v = s.Out(1);
  ...
  // =========================================================================
  Top wall(y = 1) : u = 1, v = 0 CPhysicsEquation eq_top_u;
  eq_top_u.name = "bc_top_u";
  eq_top_u.input_names = {"x", "y"};
  eq_top_u.output_names = {"u"};
  eq_top_u.residual = [](const PhysicsState &s,
                         const PhysicsData &) -> mlpdouble {
    double x = to_double(s.In(0));
    double target_u = 1.0;
    if (x < 0.1)
  ...
  auto loss_top_u = std::make_shared<CPhysicsLoss>(
      "loss_top_u", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq_top_u});

  CPhysicsEquation eq_top_v;
  eq_top_v.name = "bc_top_v";
  eq_top_v.input_names = {"x", "y"};
  eq_top_v.output_names = {"v"};
  eq_top_v.residual = [](const PhysicsState &s,
                         const PhysicsData &) -> mlpdouble {
    return s.Out(0) - 0.0;
  };
  auto loss_top_v = std::make_shared<CPhysicsLoss>(
      "loss_top_v", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq_top_v});
  ...
  eq_wall_u.residual = [](const PhysicsState &s,
                          const PhysicsData &) -> mlpdouble {
    return s.Out(0) - 0.0;
  };
  auto loss_wall_u = std::make_shared<CPhysicsLoss>(
      "loss_wall_u", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq_wall_u});

  CPhysicsEquation eq_wall_v;
  eq_wall_v.name = "bc_wall_v";
  eq_wall_v.input_names = {"x", "y"};
  eq_wall_v.output_names = {"v"};
  eq_wall_v.residual = [](const PhysicsState &s,
                          const PhysicsData &) -> mlpdouble {
    return s.Out(0) - 0.0;
  };
  auto loss_wall_v = std::make_shared<CPhysicsLoss>(
      "loss_wall_v", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq_wall_v});
