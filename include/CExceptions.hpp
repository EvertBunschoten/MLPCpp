...
public:
  DuplicatesInQueryException(const bool in_input,
                             const std::string &var) noexcept
      : is_input{in_input}, duplicate_var{var} {};
...
public:
  SharedInputsOutputsException(const std::string &var) noexcept
      : shared_var{var} {};
...
public:
  InsufficientJacobianException(const std::string &enumerators,
                                const std::string &denominators) noexcept
      : missing_enumerators{enumerators}, missing_denominators{denominators} {};
...
public:
  InsufficientHessianException(const std::string &enumerators,
                               const std::string &denominators) noexcept
      : missing_enumerators{enumerators}, missing_denominators{denominators} {};
...
public:
  JacobianNotSupportedException(const std::vector<std::string> &jacs) noexcept
      : impossible_jacobians{jacs} {};
  ~JacobianNotSupportedException() noexcept = default;
  virtual const char *what() const noexcept {
    std::string msg =
        "The following Jacobian queries were not supported by the networks: ";
...
public:
  HessianNotSupportedException(const std::vector<std::string> &hes) noexcept
      : impossible_hessians{hes} {};
  ~HessianNotSupportedException() noexcept = default;
  virtual const char *what() const noexcept {
    std::string msg =
        "The following Hessian queries were not supported by the networks: ";
...
public:
  ...
  virtual const char *what() const noexcept {
    std::string msg =
        "Number of outputs in query differs from number of requested outputs.";
    return msg.c_str();
  }
};
