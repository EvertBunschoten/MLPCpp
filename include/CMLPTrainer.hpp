...
public:
  ...
  // Modified to accept a pre-allocated PredictionResult to prevent heap
  // allocations
...
public:
  ...
  void Build() {
    ...
    for (std::size_t k = 0; k < phys_losses_.size(); ++k) {
      ...
      if (set_it == coll_sets_.end()) {
        throw std::runtime_error(
            ...
            "' references missing collocation set '" + set_name + "'.");
      }
    ...
    for (std::size_t k = 0; k < bcs_losses_.size(); ++k) {
      ...
      if (coll_sets_.find(set_name) != coll_sets_.end() &&
        ...
        // FIX: BC sets must be registered in the needs_jac/hess maps so .at()
        // doesn't throw
  ...
  TrainStepResult TrainStep() {
    ...
    const bool need_separate_grads = (annealer_ && have_data_like_terms &&
                                      st.n_phys > 0 && is_anneal_update_step);
  ...
  void TrainEpoch() {
    ...
    if (use_mpi_ && mpi_size_ > 1) {
      unsigned long long n_batches_ull =
          static_cast<unsigned long long>(n_batches);
      MPI_Allreduce(MPI_IN_PLACE, &n_batches_ull, 1, MPI_UNSIGNED_LONG_LONG,
    ...
    for (std::size_t b = 0; b < n_batches; ++b) {
      ...
      for (std::size_t k = 0; k < res.loss_phys.size(); ++k)
      ...
      for (std::size_t k = 0; k < res.loss_bcs.size(); ++k)
      ...
      for (std::size_t k = 0; k < res.lambdas.size(); ++k)
  ...
  void Train() {
    ...
    for (std::size_t epoch = 0; epoch < cfg_.max_epochs; ++epoch) {
      ...
      const double current_loss =
          (epoch_loss_count_ > 0)
              ? (cfg_.conv_metric == ConvergenceMetric::Weighted
                     ? epoch_loss_total_sum_ /
                           static_cast<double>(epoch_loss_count_)
                     : epoch_loss_sum_ / static_cast<double>(epoch_loss_count_))
              : last_result_.loss_raw;

      if (cfg_.verbose) {
        const char *metric_str = cfg_.conv_metric == ConvergenceMetric::Weighted
                                     ? "weighted"
                                     : "raw";

        std::cout << "Epoch " << (epoch + 1) << "/" << cfg_.max_epochs << " | "
      ...
      if (std::isfinite(previous_loss)) {
        ...
        if (abs_delta <= cfg_.conv_tol_abs || rel_delta <= cfg_.conv_tol_rel) {
          if (cfg_.verbose) {
...
private:
  ...
  void EvaluateReferenceLoss(typename mlpdouble::Tape &tape,
                             TrainStepState &st) {
        ...
        if (phys_it != losses_by_set_.end()) {
          for (const std::size_t k : phys_it->second) {
            ...
            if (data.empty()) {
              ...
              st.L_phys[k] +=
                  phys_losses_[k]->EvaluateOne(current_pred_, data[idx]);
            }
            ++st.n_points_seen[k];
          }
        }
  ...
  void ComputeFastPathGradient(typename mlpdouble::Tape &tape,
    ...
    if (use_mpi_ && mpi_size_ > 1) {
      MPI_Allreduce(MPI_IN_PLACE, grad_total_.data(), st.n_w, MPI_DOUBLE,
                    MPI_SUM, MPI_COMM_WORLD);
      for (double &g : grad_total_)
        g /= mpi_size_;
    }
  }
  ...
  void ComputeAnnealedPathGradient(typename mlpdouble::Tape &tape,
    ...
    for (std::size_t i = 0; i < st.n_ref; ++i) {
      zero_gradients();
      tape.clearAdjoints();
      for (std::size_t j = 0; j < st.n_ref; ++j)
    ...
    for (std::size_t k = 0; k < st.L_bcs.size(); ++k) {
      ...
      for (auto &lr : st.L_ref_vec)
        lr.setGradient(0.0);
      for (std::size_t j = 0; j < st.L_bcs.size(); ++j)
    ...
    if (use_mpi_ && mpi_size_ > 1) {
      ...
      for (double &g : grad_total_)
        g /= mpi_size_;
      for (std::size_t i = 0; i < st.n_ref + st.L_bcs.size(); ++i) {
        MPI_Allreduce(MPI_IN_PLACE, grad_per_data_term_[i].data(), st.n_w,
                      MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        for (double &g : grad_per_data_term_[i])
          g /= mpi_size_;
      }
    }
  ...
  TrainStepResult BuildTrainStepResult(const TrainStepState &st) const {
    ...
    for (std::size_t i = 0; i < st.n_ref; ++i)
      total_ref_loss += to_double(st.L_ref_vec[i]);
  ...
  void NextPhysicsBatch(const std::string &set_name) {
    ...
    if (!coll_epoch_started_.at(set_name)) {
      if (cfg_.shuffle_per_epoch && n_points > 0) {
        std::shuffle(coll_indices_[set_name].begin(),
                     coll_indices_[set_name].end(), rng_);
      }
      coll_batch_cursor_[set_name] = 0;
    ...
    for (std::size_t i = 0; i < actual; ++i) {
      batch_indices_[i] =
          coll_indices_[set_name][coll_batch_cursor_[set_name]++];
    }
  ...
  void LogStep(const TrainStepResult &r) const {
    std::cout << "Step " << step_ << " | L_ref=" << r.loss_ref
              << " | L_raw=" << r.loss_raw << " | L_total=" << r.loss_total;

    for (std::size_t k = 0; k < r.loss_phys.size(); ++k) {
      std::cout << " | phys[" << k << "]=" << r.loss_phys[k];
    }
