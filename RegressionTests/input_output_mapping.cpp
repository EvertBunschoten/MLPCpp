...
TEST_CASE("Different inputs, different outputs", "[CIOMap]") {
...
bool InputOutputMapping::DifferentInputsDifferentOutputs() {
  ...
  summary << mlp_1->GetOutputName(0) << " : " << mlp_1->GetOutput(0)
          << std::endl;
  summary << mlp_1->GetOutputName(1) << " : " << mlp_1->GetOutput(1)
          << std::endl;
  summary << "Network 2: " << std::endl;
  summary << mlp_2->GetOutputName(0) << " : " << mlp_2->GetOutput(0)
          << std::endl;
  summary << mlp_2->GetOutputName(1) << " : " << mlp_2->GetOutput(1)
          << std::endl;
...
bool InputOutputMapping::SameInputsDifferentOutputs() {
  ...
  MLPToolbox::CNeuralNetwork *mlp_1 =
      CreateRandomNetwork(network_inputs, output_names_1);
...
bool InputOutputMapping::NullOutputs() {
  ...
  bool passed_test = (val_out_1 == 0.0) && (val_out_2 == mlp_1->GetOutput(1)) &&
                     (val_out_3 == 0.0) && (val_out_4 == 0.0) &&
                     (val_out_5 == 0.0);
...
bool InputOutputMapping::VectorInputOutputs() {
  ...
  if (inside_m != inside_v) {
    passed_test = false;
    summary << "Member-wise and vector-wise query input returns different "
               "inclusion.\n";
  }
  if (val_x_v != val_x_m && val_y_v != val_y_m && val_z_v != val_z_m) {
    ...
    summary << "Member-wise and vector-wise query input returns different "
               "output:\n";
    summary << "Member-wise: x:" << val_x_m << " y: " << val_y_m
            << " z: " << val_z_m << std::endl;
    summary << "Vector-wise: x:" << val_x_v << " y: " << val_y_v
            << " z: " << val_z_v << std::endl;
  }
  if (val_null_m != 0.0) {
    passed_test = false;
    summary << "Member-wise null assignment returns non-zero value: "
            << val_null_m << std::endl;
  }
  if (val_null_v != 0.0) {
    passed_test = false;
    summary << "Vector-wise null assignment returns non-zero value: "
            << val_null_v << std::endl;
  }
  delete mlp_1;
  delete mlp_2;
...
bool InputOutputMapping::RunTest() {
  ...
  passed = (test_multiple_inputs && test_multiple_outputs && test_mimo &&
            test_null && test_vectorwise);
  return passed;
};
