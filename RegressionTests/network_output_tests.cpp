...
bool OutputCorrectness::CopyConstructorTest() {
  ...
  if (!passed)
    summary << "From copy constructor: " << (passed ? "Passed" : "Failed")
            << std::endl;
  return passed;
}
...
bool OutputCorrectness::FileWriterReaderTest() {
  ...
  if (!passed)
    summary << "From writing-reading input file: "
...
bool OutputCorrectness::WeightsBiasesTest() {
  ...
  if (!passed)
    summary << "From weights and biases test: "
            << (passed ? "Passed" : "Failed") << std::endl;
  return passed;
}
...
bool OutputCorrectness::VectorInputOutputs() {
  ...
  if (!passed_test) {
    ...
    for (auto iInput = 0u; iInput < mlp->GetnInputs(); iInput++)
      summary << mlp->GetInputName(iInput) << " : "
              << network_inputs_vec[iInput] << std::endl;
    summary << "Network output from vector input: " << output_ref << std::endl;
    summary << "Network output from piece-wise input: " << output_p
            << std::endl;
  }
  delete mlp;
  return passed_test;
