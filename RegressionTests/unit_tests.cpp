...
int main() {
  ...
  size_t n_passed =
      std::accumulate(passed_tests.begin(), passed_tests.end(), 0);
  for (auto test : unit_tests) {
    std::cout << test->GetTag() << " : "
              << (test->did_pass() ? "passed" : "FAILED") << std::endl;
  }
  std::cout << "Finished unit tests. Passed: " << n_passed << "/"
            << passed_tests.size() << std::endl;
