...
TEST_CASE("Network Jacobian differentiation test", "[CNeuralNetwork]") {
    /*! \brief Check whether the Jacobian of the network output equals the differentiated network output. */
...
TEST_CASE("Network Hessian differentiation test", "[CNeuralNetwork]") {
    /*! \brief Check whether the Hessian of the network output equals the differentiated network output. */
    std::vector<std::string> input_names = {"a", "b"}, 
                             output_names = {"x", "y"};
    MLPToolbox::CNeuralNetwork * mlp = CreateRandomNetwork(input_names, output_names);
    mlpdouble val_a = 0.5, val_b = 2.0;
