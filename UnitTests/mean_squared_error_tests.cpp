...
#define REQUIRE_EQUAL_TOL(a, b, tol) \
    REQUIRE(static_cast<double>(a) == Catch::Approx(static_cast<double>(b)).margin(tol))

// Helper to construct PredictionResult with inputs and outputs for testing
// Note: Derivatives (jacobian/hessian) are omitted as they are not used by MSE loss
...
TEST_CASE("CMeanSquaredErrorLoss empty predictions", "[CMeanSquaredErrorLoss]") {
...
TEST_CASE("CMeanSquaredErrorLoss size mismatch throws", "[CMeanSquaredErrorLoss]") {
    MLPToolbox::CMeanSquaredErrorLoss loss;
...
TEST_CASE("CMeanSquaredErrorLoss inconsistent outputs throws", "[CMeanSquaredErrorLoss]") {
    MLPToolbox::CMeanSquaredErrorLoss loss;
...
TEST_CASE("CMeanSquaredErrorLoss ref_data size mismatch throws", "[CMeanSquaredErrorLoss]") {
    MLPToolbox::CMeanSquaredErrorLoss loss;
    std::vector<MLPToolbox::PredictionResult> preds(1);
    preds[0].outputs = {1.0, 2.0};
...
TEST_CASE("CMeanSquaredErrorLoss simple MSE calculation", "[CMeanSquaredErrorLoss]") {
    MLPToolbox::CMeanSquaredErrorLoss loss;
    std::vector<MLPToolbox::PredictionResult> preds = {
...
TEST_CASE("CMeanSquaredErrorLoss multiple points MSE", "[CMeanSquaredErrorLoss]") {
    MLPToolbox::CMeanSquaredErrorLoss loss;
    std::vector<MLPToolbox::PredictionResult> preds = {
        make_pred({0.0}, {2.0}),
        make_pred({1.0}, {4.0})
