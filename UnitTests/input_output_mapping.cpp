...
TEST_CASE("Different queries, same network", "[CIOMap]") {
  /*! \brief Different queries with the same network should return the same
   * values. */

  /* Create two identical networks with different input and output variable
   * names. */
  std::vector<std::string> input_names_1 = {"a", "b", "c"},
                           output_names_1 = {"x", "z"};
  MLPToolbox::CNeuralNetwork *mlp_1 =
      CreateRandomNetwork(input_names_1, output_names_1);
  ...
  /* Define two queries for the two sets of inputs-outputs that refer to the
   * same variables. */
...
TEST_CASE("Query with multiple networks", "[CIOMap]") {
  /*! \brief Single query for networks with multiple networks and input/output
   * variables. */
...
TEST_CASE("Null queries", "[CIOMap]") {
  ...
  double val_in_1, val_in_2, val_out_1{1.0}, val_out_2{1.0}, val_out_3{1.0},
      val_out_4{1.0}, val_out_5{1.0};
...
TEST_CASE("Jacobian and Hessian queries", "[CIOMap]") {
  ...
  MLPToolbox::CNeuralNetwork *mlp_1 =
      CreateRandomNetwork(input_names_1, output_names_1);
  MLPToolbox::CNeuralNetwork *mlp_2 =
      CreateRandomNetwork(input_names_2, output_names_2);
...
TEST_CASE("Input-output accessors through vectors", "[CIOMap]") {
  ...
  double val_a, val_b, val_x_m, val_y_m, val_z_m, val_null_m, val_x_v, val_y_v,
      val_z_v, val_null_v;
  val_a = 0.2;
  val_b = 0.8;
  ...
  std::vector<double *> refs_out_vec = {&val_x_v, &val_y_v, &val_z_v,
                                        &val_null_v};
  ...
  bool inside_v =
      mlp_collection.Predict(query_vector, vals_in_vec, refs_out_vec);

  REQUIRE_EQUAL_TOL(val_x_v, val_x_m, 1e-12);
  REQUIRE_EQUAL_TOL(val_y_v, val_y_m, 1e-12);
  REQUIRE_EQUAL_TOL(val_z_v, val_z_m, 1e-12);
...
TEST_CASE("Ill-defined queries for look-up", "[CIOMap]") {
...

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(superfluous_outputs),
                      MLPToolbox::QueryOutputNotFoundException);
  }

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(superfluous_inputs),
                      MLPToolbox::QueryInputNotFoundException);
  }

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(duplicate_inputs),
                      MLPToolbox::DuplicatesInQueryException);
  }

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(duplicate_outputs),
                      MLPToolbox::DuplicatesInQueryException);
  }

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(shared_variables),
                      MLPToolbox::SharedInputsOutputsException);
  }

  {
    ...
    REQUIRE_THROWS_AS(
        mlp_collection.PairVariableswithMLPs(insufficient_jacobian),
        MLPToolbox::InsufficientJacobianException);
  }
  {
    ...
    REQUIRE_THROWS_AS(
        mlp_collection.PairVariableswithMLPs(insufficient_hessian),
        MLPToolbox::InsufficientHessianException);
  }
  delete mlp_1;
}
...
TEST_CASE("Ill-defined queries for Jacobians and Hessians", "[CIOMap]") {
...

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(illdefined_jacobian),
                      MLPToolbox::JacobianNotSupportedException);
  }

  {
    ...
    REQUIRE_THROWS_AS(mlp_collection.PairVariableswithMLPs(illdefined_hessian),
                      MLPToolbox::HessianNotSupportedException);
  }
  delete mlp_1;
  delete mlp_2;
}
