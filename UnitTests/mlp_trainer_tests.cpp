...
#define REQUIRE_EQUAL_TOL(a, b, tol)                                           \
  REQUIRE(to_double(a) == Catch::Approx(to_double(b)).margin(tol))
...
TEST_CASE("CMLPTrainer constructor validation", "[CMLPTrainer]") {
  ...
  REQUIRE_THROWS_AS(CMLPTrainer(net, adam, annealer_cfg, cfg_neg_abs),
                    std::invalid_argument);

  TrainerConfig cfg_neg_rel;
  cfg_neg_rel.conv_tol_rel = -1e-6;
  REQUIRE_THROWS_AS(CMLPTrainer(net, adam, annealer_cfg, cfg_neg_rel),
                    std::invalid_argument);

  TrainerConfig cfg_neg_grad;
  cfg_neg_grad.max_grad_norm = -1.0;
  REQUIRE_THROWS_AS(CMLPTrainer(net, adam, annealer_cfg, cfg_neg_grad),
                    std::invalid_argument);
}

TEST_CASE("CMLPTrainer data fitting loss management", "[CMLPTrainer]") {
  ...
  REQUIRE_NOTHROW(trainer.AddFittingLoss(custom_loss));
  REQUIRE_THROWS_AS(trainer.AddFittingLoss(nullptr), std::invalid_argument);
  ...
  REQUIRE_THROWS_AS(trainer.AddFittingLoss(custom_loss), std::runtime_error);
  REQUIRE_THROWS_AS(trainer.ClearFittingLosses(), std::runtime_error);
}
...
TEST_CASE("CMLPTrainer training data validation", "[CMLPTrainer]") {
  ...
  REQUIRE_THROWS_AS(trainer.SetTrainingData(X, Y_bad_size),
                    std::invalid_argument);
  REQUIRE_THROWS_AS(trainer.SetTrainingData(X, Y_bad_dim),
                    std::invalid_argument);
...
TEST_CASE("CMLPTrainer collocation points validation", "[CMLPTrainer]") {
  ...
  REQUIRE_THROWS_AS(trainer.SetCollocationPoints("", {{0.5}}),
                    std::invalid_argument);
  REQUIRE_THROWS_AS(trainer.SetCollocationPoints("test", {{0.5, 0.5}}),
                    std::invalid_argument);
  ...
  trainer.FinalizeConfiguration();
  REQUIRE_THROWS_AS(trainer.SetCollocationPoints("test2", {{0.5}}),
                    std::runtime_error);
}
...
TEST_CASE("CMLPTrainer physics loss validation", "[CMLPTrainer]") {
  ...
  eq.input_names = {"x"};
  eq.output_names = {"y"};
  eq.residual = [](const PhysicsState &, const PhysicsData &) {
    return mlpdouble(0.0);
  };

  auto phys_loss = std::make_shared<CPhysicsLoss>(
      "phys", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq});

  REQUIRE_THROWS_AS(trainer.AddPhysicsLoss(nullptr, "colloc"),
                    std::invalid_argument);
  REQUIRE_THROWS_AS(trainer.AddPhysicsLoss(phys_loss, ""),
                    std::invalid_argument);
  REQUIRE_THROWS_AS(trainer.AddPhysicsLoss(phys_loss, "non_existent"),
                    std::invalid_argument);

  REQUIRE_NOTHROW(trainer.AddPhysicsLoss(phys_loss, "colloc"));
  REQUIRE_THROWS_AS(trainer.AddPhysicsLoss(phys_loss, "colloc"),
                    std::invalid_argument);
...
TEST_CASE("CMLPTrainer physics data validation", "[CMLPTrainer]") {
  ...
  auto phys_loss = std::make_shared<CPhysicsLoss>(
      "phys", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{"src"}, std::vector<CPhysicsEquation>{eq});
  ...
  REQUIRE_THROWS_AS(trainer.SetPhysicsData("phys", bad_rows),
                    std::invalid_argument);

  std::vector<std::vector<mlpdouble>> bad_size = {{1.0, 2.0}, {3.0, 4.0}};
  REQUIRE_THROWS_AS(trainer.SetPhysicsData("phys", bad_size),
                    std::invalid_argument);

  REQUIRE_THROWS_AS(trainer.SetPhysicsData("non_existent", valid_data),
                    std::runtime_error);
}

TEST_CASE("CMLPTrainer FinalizeConfiguration validation", "[CMLPTrainer]") {
  std::vector<std::size_t> arch = {1, 2, 1};
  CNeuralNetwork net(arch);
  net.SetInputName(0, "x");
  net.SetOutputName(0, "y");
  ...
  REQUIRE_THROWS_AS(trainer.FinalizeConfiguration(), std::runtime_error);

  std::vector<std::vector<mlpdouble>> X = {{0.0}};
  std::vector<std::vector<mlpdouble>> Y = {{0.0}};
  trainer.SetTrainingData(X, Y);
  trainer.AddFittingLoss(std::make_shared<CMeanSquaredErrorLoss>());
  REQUIRE_NOTHROW(trainer.FinalizeConfiguration());
}
...
TEST_CASE("CMLPTrainer gradient clipping", "[CMLPTrainer]") {
  ...
  auto phys_loss = std::make_shared<CPhysicsLoss>(
      "dy_dx", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq});
...
TEST_CASE("CMLPTrainer multiple physics losses", "[CMLPTrainer]") {
  ...
  CAdam adam(1e-3, 0.9, 0.999, 1e-8);
  AnnealerConfig annealer_cfg;
  annealer_cfg.n_data_terms =
      1; // Trainer will override this based on added ref losses
  TrainerConfig cfg;
  cfg.max_epochs = 2;
  cfg.batch_size = 2;
  cfg.verbose = false;
  ...
  auto phys_loss1 = std::make_shared<CPhysicsLoss>(
      "phys1", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq1});

  auto phys_loss2 = std::make_shared<CPhysicsLoss>(
      "phys2", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq2});
...
TEST_CASE("CMLPTrainer multiple data fitting losses", "[CMLPTrainer]") {
  ...
  // Add two separate data fitting losses
  trainer.AddFittingLoss(std::make_shared<CMeanSquaredErrorLoss>());
  trainer.AddFittingLoss(std::make_shared<CMeanSquaredErrorLoss>());
  ...
  auto phys_loss = std::make_shared<CPhysicsLoss>(
      "dy1_dx", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{}, std::vector<CPhysicsEquation>{eq});
...
TEST_CASE("CMLPTrainer physics mini-batch limits points per step",
          "[CMLPTrainer]") {
  std::vector<std::size_t> arch = {1, 4, 1};
  CNeuralNetwork net(arch);
  net.SetInputName(0, "x");
  ...
  eq.residual = [&call_count](const PhysicsState &,
...
TEST_CASE("CMLPTrainer missing physics data throws at runtime",
          "[CMLPTrainer]") {
  std::vector<std::size_t> arch = {1, 2, 1};
  CNeuralNetwork net(arch);
  net.SetInputName(0, "x");
  ...
  eq.output_names = {"y"};
  eq.requires_jacobian = true;
  eq.residual = [](const PhysicsState &s,
                   const PhysicsData &data) -> mlpdouble {
    return s.Jac(0, 0) - data.Ref("source");
  };

  auto phys_loss = std::make_shared<CPhysicsLoss>(
      "dy_dx_source", net.GetInputVars(), net.GetOutputVars(),
      std::vector<std::string>{"source"}, std::vector<CPhysicsEquation>{eq});
