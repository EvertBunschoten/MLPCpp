...
TEST_CASE("CGradientAnnealer basic update logic", "[CGradientAnnealer]") {
    ...
    CHECK(annealer.get_lambda_hat(0) == Approx(4.0));
    // lambda = (1 - 0.5) * 1.0 + 0.5 * 4.0 = 2.5
    CHECK(annealer.get_lambda(0) == Approx(2.5));
...
TEST_CASE("CGradientAnnealer handles zero data mean", "[CGradientAnnealer]") {
    ...
    CHECK(annealer.get_lambda_hat(0) == Approx(cfg.lambda_max));
    // lambda = 0.1 * 1.0 + 0.9 * 10.0 = 90.1
    CHECK(annealer.get_lambda(0) == Approx(9.1));
...
TEST_CASE("CGradientAnnealer enforces clamping", "[CGradientAnnealer]") {
    ...
    SECTION("Upper clamp") {
        ...
        CHECK(annealer.get_lambda(0) == Approx(2.0));
    }
    ...
    SECTION("Lower clamp") {
        ...
        CHECK(annealer.get_lambda(0) == Approx(0.5));
    }
}

TEST_CASE("CGradientAnnealer handles multiple terms and EMA accumulation", "[CGradientAnnealer]") {
    AnnealerConfig cfg;
    cfg.n_data_terms = 2;
    cfg.alpha = 0.5;
    cfg.lambda_init = 1.0;
    ...
    CHECK(annealer.get_lambda(0) == Approx(2.5));
    CHECK(annealer.get_lambda(1) == Approx(1.5));

    // step2
    annealer.update(ref, {data1, data2});
    CHECK(annealer.get_lambda(0) == Approx(3.25));
    CHECK(annealer.get_lambda(1) == Approx(1.75));
...
TEST_CASE("CGradientAnnealer reset and accessors", "[CGradientAnnealer]") {
    ...
    CHECK(annealer.config().alpha == Approx(0.8));
    ...
    for(std::size_t i = 0; i < 3; ++i) {
        CHECK(annealer.get_lambda(i) == Approx(cfg.lambda_init));
        CHECK(annealer.get_lambda_hat(i) == Approx(cfg.lambda_init));
    }
